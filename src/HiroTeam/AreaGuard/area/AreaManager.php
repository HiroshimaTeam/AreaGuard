...
class AreaManager {
	...
	/** @var Area[] */
	private array $area = [];
	private array $processCreationNewArea = [];
	...
	public function onNewAreaCreation(Player $player, string $areaName, bool $expandVertically, array $settings = []) : void {
	...
	public function onInterruptCreateArea(Player $player) : void {
		$playerName = $player->getName();
		if (isset($this->processCreationNewArea[$playerName])) {
			unset($this->processCreationNewArea[$playerName]);
		}
	...
	private function createNewArea(string $areaName, Position $position1, Position $position2, array $settings = []) : void {
	 ...
	 * @throws JsonException
	 */
	public function deleteArea(string $areaName) : void {
